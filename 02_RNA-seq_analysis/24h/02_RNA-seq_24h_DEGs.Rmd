...
```{r Import count file}
counts <- read.table(file="../../results/featureCount/readCounts.txt",
...
pdf("RNA-seq_24h_MALAT1_boxplot_v1.pdf")
par(mfrow=c(1,2)) 
ggplot(geneCounts, aes(x=group, y=count, color=group, group=group)) +
...
svg("RNA-seq_24h_MALAT1_boxplot_v1.svg")
par(mfrow=c(1,2)) 
ggplot(geneCounts, aes(x=group, y=count, color=group, group=group)) +
  scale_y_log10() + geom_boxplot(outlier.colour="black", outlier.shape=16,
             outlier.size=2, notch=FALSE) + theme_minimal()
...
pdf("RNA-seq_24h_NR4A1_boxplot_v1.pdf")
par(mfrow=c(1,2)) 
ggplot(geneCounts, aes(x=group, y=count, color=group, group=group)) +
  scale_y_log10() + geom_boxplot(outlier.colour="black", outlier.shape=16,
             outlier.size=2, notch=FALSE) + theme_minimal()
...
save(dds,file = "RNA-seq_24h_dds_object.RData")
...
write.csv(resOrderedDF, file="../../results/24h_DEGs_results.csv")

library("ReportingTools")
htmlRep <- HTMLReport(shortName="MALAT1_DEGs_report", title="MALAT1 DEGs",
		        reportDirectory="/results/DEG/DEGs_report")
