...
#Run as: bash ./01_RNA-seq_24h_analysis.sh

set -e

echo "----- RNA-seq 24h PIPELINE (PE) -----"

project_folder="/01_Projects/lncRNA_project/RNA-seq_HeLa_24h"
...
cd $project_folder
mkdir -p raw_data results trimmed logs scripts aligned
...
Rscript -e "rmarkdown::render('$project_folder/scripts/02_RNA-seq_24h_DEGs.Rmd',params=list(args = myarg))"
